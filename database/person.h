...
namespace database
{
    struct Person
    {
        ...
        static void init()
        {
            try
            {
                std::cout << "initializing person: ";
                std::string query = "CREATE TABLE IF NOT EXISTS person (`login` varchar(256) NOT NULL,`password_hash` varchar(1024) NOT NULL,`first_name` varchar(1024) DEFAULT NULL,`last_name` varchar(1024) DEFAULT NULL,`age` int DEFAULT NULL,`hobby` varchar(4096) DEFAULT NULL,`city` varchar(1024) DEFAULT NULL, PRIMARY KEY (`login`))";
        ...
        static bool check_exist(std::string &login)
        {
            ...
            database::Database_MySQL::get().query(
                query, [&](int row, int column, std::string value) {
                    if ((row ==0)&&(column==0)) count = atoi(value.c_str()); }, [](int) {});

            if (count < 0)
                throw std::logic_error("error executing query");
            return (count > 0);
        }
