...
namespace database
{
    struct Friends
    {
        ...
        static void init()
        {
            try
            {
                std::cout << "initializing friends: " ;
                std::string query = "CREATE TABLE IF NOT EXISTS friends (`id` int NOT NULL AUTO_INCREMENT,`source_login` VARCHAR(256) NOT NULL,`destination_login` VARCHAR(256) NOT NULL,PRIMARY KEY (`id`))";
        ...
        static std::vector<Friends> get_friends(const std::string &source_login)
        {
            ...
            database::Database_MySQL::get().query(
                query, [&](int, int column, std::string value) {
                    ...
                    {
                    case 0:
                        record.destination_login = value;
                        break;
                    }
                },
                [&](int) {
